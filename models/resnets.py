...
__all__ = ['resnet20s', 'resnet32s', 'resnet44s', 'resnet56s']
...
class ResNets(nn.Module):
    def __init__(self, conv_layer, linear_layer, block, num_blocks, num_classes=10, k=0.01):
    ...
    # Reserve the top k kernels in each layer w.r.t. the L1 norm.
    # We only prune the non-short cut conv layer.
    def forward(self, x):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
...
def resnet20s(conv_layer, linear_layer, init_type, **kwargs):
...
def resnet32s(conv_layer, linear_layer, init_type, **kwargs):
    assert init_type == "kaiming_normal", "only supporting default init for Resnets"
    return ResNets(conv_layer, linear_layer, BasicBlock, [5, 5, 5], **kwargs)


def resnet44s(conv_layer, linear_layer, init_type, **kwargs):
    assert init_type == "kaiming_normal", "only supporting default init for Resnets"
    return ResNets(conv_layer, linear_layer, BasicBlock, [7, 7, 7], **kwargs)


def resnet56s(conv_layer, linear_layer, init_type, **kwargs):
    assert init_type == "kaiming_normal", "only supporting default init for Resnets"
    return ResNets(conv_layer, linear_layer, BasicBlock, [9, 9, 9], **kwargs)


def test():
    net = resnet20s(nn.Conv2d, nn.Linear, "kaiming_normal")
